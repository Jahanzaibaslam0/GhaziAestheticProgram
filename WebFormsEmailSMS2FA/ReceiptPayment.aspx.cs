...
namespace WebFormsEmailSMS2FA
{
    public partial class ReceiptPayment : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                        ...
                        if (count > 0)
                        {
                            ...
                            using (SqlConnection conn = new SqlConnection(sConnectionString))
                            {
                                ...
                                while (dReader.Read())
                                {
                                    ...
                                    lblName1.Text = dReader["FirstName"].ToString() + " " + dReader["LastName"].ToString();
                                    lblPaymentMode1.Text = dReader["PaymentType"].ToString();
                                    //ddlPayMode.Items.FindByText(dReader["PaymentType"].ToString()).Selected = true;
                                    lblPaymentDate.Text = String.Format("{0:dddd, MMMM d, yyyy}", dReader["PaymentDate"]);  // "Sunday, March 9, 2008"dReader["PaymentDate"].ToString();
            ...
            lblRegistrationAmount.Text = Registration.ToString() + "/-";
            lblWorkshopAmount.Text = workshop.ToString() + "/-";
            lblTotalAmount.Text = (Registration + workshop).ToString() + "/-";
            lblAmountinWords1.Text = this.ConvertNumbertoWords(workshop + Registration) + " RUPEES ONLY";//Payment.ToString();
